...
async function verifyUserToken(req, userId) {
...
export default async function handler(req, res) {
  ...
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, X-Notify-User, Authorization');
  ...
  // It does not require the payer to have Telegram linked.
  if (type === 'payment_request_admin') {
    ...
    const [payerR, adminsR] = await Promise.all([
      ...
      fetch(`${SB_URL}/rest/v1/profiles?role=eq.admin&tg_linked=is.true&tg_chat_id=not.is.null&select=tg_chat_id,tg_username`, {
        headers: { apikey: SB_KEY, Authorization: `Bearer ${SB_KEY}`, Accept: 'application/json' },
      }),
    ]);
    const payerRows = await payerR.json().catch(() => []);
    const admins = await adminsR.json().catch(() => []);
    ...
    const lines = [
      '<b>💳 Новая заявка на оплату Screener</b>',
      '—',
      `👤 <b>${escHtml(payerName)}</b>`,
      `Telegram: <b>${escHtml(tg || 'не указан')}</b>`,
      `Тариф: <b>${plan}</b>`,
      `Сумма: <b>${Number.isFinite(amount) ? amount : 0} USDT</b>`,
      'Сеть: <b>TON · USDT</b>',
      `TX hash: <code>${escHtml(tx || 'не указан')}</code>`,
      paymentId ? `Payment ID: <code>${escHtml(paymentId)}</code>` : '',
      '',
      `<a href="${APP_URL}/admin">Открыть /admin и подтвердить →</a>`,
    ].filter(Boolean);

    let sent = 0;
    for (const admin of admins) {
      if (!admin?.tg_chat_id) continue;
      if (await tgSend(admin.tg_chat_id, lines.join('\n'))) sent++;
    }
    return res.status(sent ? 200 : 502).json({ ok: sent > 0, sent });
  }
