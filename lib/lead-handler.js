...
function isAllowedOrigin(req) {
  ...
  const configuredOrigins = String(APP_URL).split(',').map((item) => item.trim().replace(/\/$/, '')).filter(Boolean);
...
export async function handleLead(req, res) {
  ...
  if (!isAllowedOrigin(req)) return res.status(403).json({ ok: false, error: 'Origin not allowed' });
  const contentLength = Number(req.headers['content-length'] || 0);
  if (contentLength > 12_000) return res.status(413).json({ ok: false, error: 'Request too large' });

  let body;
  try { body = parseBody(req); }
  catch { return res.status(400).json({ ok: false, error: 'Некорректный формат заявки.' }); }

  if (cleanText(body.company || body.website, 120)) return res.status(200).json({ ok: true });
  ...
  if (!telegram) return res.status(400).json({ ok: false, error: 'Укажите Telegram в формате @username или t.me/username.' });
  if (!ALLOWED_INTERESTS.has(interest)) return res.status(400).json({ ok: false, error: 'Выберите интерес из списка.' });
  if (!ALLOWED_EXPERIENCE.has(experience)) return res.status(400).json({ ok: false, error: 'Выберите опыт из списка.' });
  if (goal.length < 10) return res.status(400).json({ ok: false, error: 'Опишите задачу чуть подробнее.' });

  const ip = requestIp(req);
  if (isRateLimited(ip)) return res.status(429).json({ ok: false, error: 'Слишком много заявок. Попробуйте снова через несколько минут.' });
  if (!BOT_TOKEN) {
  ...
  try {
    ...
    const sendToTelegram = async (chatId) => {
      const response = await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
        ...
        body: JSON.stringify({ chat_id: chatId, ...telegramPayload }),
      });
    ...
    if (!delivery.response.ok || !delivery.result.ok) {
      console.warn('[lead] Telegram rejected message:', delivery.result.description || delivery.response.status);
      return res.status(502).json({ ok: false, error: 'Не удалось доставить заявку. Попробуйте ещё раз.' });
    }
