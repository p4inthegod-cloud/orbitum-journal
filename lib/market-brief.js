...
export function buildChannelMessage(analysis, appUrl = '') {
  const symbol = escapeHtml(analysis.symbol);
  const link = appUrl
    ? `\n\n<a href="${escapeHtml(appUrl.replace(/\/$/, ''))}/screener?coin=${encodeURIComponent(`${analysis.symbol}/USDT`)}">📈 Открыть график в Eternity Trade</a>`
    : '';
  ...
  return (
    ...
    `Open Interest · ${formatMoney(analysis.openInterestValue)}  ${oiChange}\n` +
...
export function buildChatMessage(analysis, appUrl = '') {
  const link = appUrl
    ? `\n<a href="${escapeHtml(appUrl.replace(/\/$/, ''))}/screener?coin=${encodeURIComponent(`${analysis.symbol}/USDT`)}">Открыть график →</a>`
    : '';
